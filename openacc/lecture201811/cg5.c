...
#pragma acc data copyin(z[N], dd[N], r[N], p[N], q[N], A[N*N]) copy(x[N])
  for(iter=1; iter<=maxiter; iter++){
	...
	// {z} = [Minv]{r}
#pragma acc kernels present(z,dd,r)
#pragma acc loop independent
	...
	// {rho} = {r}{z}
	rho = 0.0;
#pragma acc kernels present(r,z)
#pragma acc loop independent
	for(i=0;i<N;i++){
	  rho += r[i]*z[i];
	}
	...
	if(iter==1){
#pragma acc kernels present(p,z)
#pragma acc loop independent
	  for(i=0;i<N;i++){
		p[i] = z[i];
	  }
...
#pragma acc loop independent
	...
	// {q} = [A]{p}
#pragma acc kernels present(q,A,p)
#pragma acc loop independent
	for(i=0;i<N;i++){
	  q[i] = 0.0;
	  for(j=0;j<N;j++){
...
#pragma acc loop independent
	...
	// {x} = {x} + alpha*{p}
	// {r} = {r} - alpha*{q}
#pragma acc kernels present(x,p,r,q)
#pragma acc loop independent
	for(i=0;i<N;i++){
	  x[i] += + alpha*p[i];
	  r[i] += - alpha*q[i];
