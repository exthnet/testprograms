...
__global__ void gpukernelW32(double *out, double *in, int iloop)
{
  ...
  for(i=0;i<iloop;i++){
	tmp = 0.0;
	for(y=0; y<32; y++){
	  tmp += in[y*32000+threadIdx.x + i*32];
	}
	for(int offset=16; offset>0; offset/=2){
	  tmp += __shfl_down_sync(0xffffffff, tmp, offset, 32);
	}
	if(threadIdx.x==0)out[0] += tmp;
  }
}
...
__global__ void gpukernelW16a(double *out, double *in, int iloop)
{
  ...
  for(i=0;i<iloop;i++){
	...
	for(y=0; y<32; y+=2){
	  for(x=0; x<32; x+=16){
		tmp += in[(y+threadIdx.x/16)*32000 + threadIdx.x%16+x + i*32];
	  }
	}
	for(int offset=8; offset>0; offset/=2){
	  tmp += __shfl_down_sync(0xffffffff, tmp, offset, 32);
	}
	if(threadIdx.x%16==0)out[threadIdx.x/16] += tmp;
  }
}
...
__global__ void gpukernelW8a(double *out, double *in, int iloop)
{
  ...
  for(i=0;i<iloop;i++){
	...
	for(y=0; y<32; y+=4){
	  for(x=0; x<32; x+=8){
		tmp += in[(y+threadIdx.x/8)*32000 + threadIdx.x%8+x + i*32];
	  }
	}
	for(int offset=4; offset>0; offset/=2){
	  tmp += __shfl_down_sync(0xffffffff, tmp, offset, 32);
	}
	if(threadIdx.x%8==0)out[threadIdx.x/8] += tmp;
  }
}
...
__global__ void gpukernelW4a(double *out, double *in, int iloop)
{
  ...
  for(i=0;i<iloop;i++){
	...
	for(y=0; y<32; y+=8){
	  for(x=0; x<32; x+=4){
		tmp += in[(y+threadIdx.x/4)*32000 + threadIdx.x%4+x + i*32];
	  }
	}
	for(int offset=2; offset>0; offset/=2){
	  tmp += __shfl_down_sync(0xffffffff, tmp, offset, 32);
	}
	if(threadIdx.x%4==0)out[threadIdx.x/4] += tmp;
  }
}
...
__global__ void gpukernelW2a(double *out, double *in, int iloop)
{
  ...
  for(i=0;i<iloop;i++){
	...
	for(y=0; y<32; y+=16){
	  for(x=0; x<32; x+=2){
		tmp += in[(y+threadIdx.x/2)*32000 + threadIdx.x%2+x + i*32];
	  }
	}
	for(int offset=1; offset>0; offset/=2){
	  tmp += __shfl_down_sync(0xffffffff, tmp, offset, 32);
	}
	if(threadIdx.x%2==0)out[threadIdx.x/2] += tmp;
  }
}
...
__global__ void gpukernelW1(double *out, double *in, int iloop)
{
  ...
  for(i=0;i<iloop;i++){
	tmp = 0.0;
	for(x=0; x<32; x++){
	  tmp += in[threadIdx.x*32000+x + i*32];
	}
	out[threadIdx.x] += tmp;
  }
}
...
__global__ void gpukernelWNa(double *out, double *in, int iloop)
{
  ...
  for(i=0;i<iloop;i++){
	...
	for(y=0; y<32; y+=32/N){
	  for(x=0; x<32; x+=N){
		tmp += in[(y+threadIdx.x/N)*32000 + threadIdx.x%N+x + i*32];// * vec[x];
	  }
	}
	for(int offset=N/2; offset>0; offset/=2){
	  tmp += __shfl_down_sync(0xffffffff, tmp, offset, 32);
	}
	if(threadIdx.x%N==0)out[threadIdx.x/N] += tmp;
  }
}
template<int N>
