...
__global__ void gpukernel1(double *out, double *in, int id)
{
  int y;
  double tmp = 0.0;
  for(y=0; y<32; y++){
	tmp += in[y*32+threadIdx.x + id*32*32];
  }
  out[threadIdx.x] += tmp;
}

__global__ void gpukernel2(double *out, double *in, int id)
{
  int x;
  double tmp = 0.0;
  for(x=0; x<32; x++){
	tmp += in[threadIdx.x*32+x + id*32*32];
  }
  out[threadIdx.x] += tmp;
}
