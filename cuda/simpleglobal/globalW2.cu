...
__global__ void gpukernelH16r(double *out, double *mat, double *vec, int iloop)
{
  ...
  for(i=0;i<iloop;i++){
	for(y=0; y<32; y+=16){
	  ...
	  int offset=16;{ //for(int offset=16; offset>0; offset/=2){
		tmp += __shfl_down_sync
		  (0xffffffff, tmp, offset, 32);
	  }
  ...
  for(i=0;i<iloop;i++){
	for(x=0; x<32; x+=2){
	  for(y=0; y<32; y+=16){
		tmp = 0.0;
		tmp += mat[(y+threadIdx.x%16)*32 + threadIdx.x/16+x + i*32*32];// * vec[threadIdx.x/16+x];
		int offset=16;{ //for(int offset=8; offset>0; offset/=2){
		  tmp += __shfl_down_sync
			(0xffffffff, tmp, offset, 32);
		}
		if(threadIdx.x/16==0)out[y+threadIdx.x] += tmp;
...
__global__ void gpukernelH8r(double *out, double *mat, double *vec, int iloop)
{
  ...
  for(i=0;i<iloop;i++){
	for(y=0; y<32; y+=8){
	  ...
	  for(int offset=16; offset>=8; offset/=2){
		tmp += __shfl_down_sync
		  (0xffffffff, tmp, offset, 32);
	  }
...
__global__ void gpukernelH2r(double *out, double *mat, double *vec, int iloop)
{
  ...
  for(i=0;i<iloop;i++){
	for(y=0; y<32; y+=2){
	  ...
	  for(int offset=16; offset>=2; offset/=2){
		tmp += __shfl_down_sync
		  (0xffffffff, tmp, offset, 32);
	  }
