...
#define ALLOCATE(x,y) CHK_DO(cudaMallocHost((void**)&x,sizeof(double)*N*N));CHK_DO(cudaMalloc((void**)&y, sizeof(double)*N*N));
//, cudaMemAttachGlobal))
...
int main(int argc, char **argv)
{
  ...
  gpukernel<<<M,1024,0,s1>>>(N,dA1,dB1,dC1);
  ...
  gpukernel<<<M,1024,0,s2>>>(N,dA2,dB2,dC2);
  ...
  gpukernel<<<M,1024,0,s4>>>(N,dA4,dB4,dC4);
  ...
  gpukernel<<<M,1024,0,s3>>>(N,dA3,dB3,dC3);
