...
int RANDSEED = 0;
#ifdef _USE_AVX3
__declspec(align(64)) double *g_A, *g_B, *g_C, *g_R;
#else
#ifdef _USE_AVX
__declspec(align(32)) double *g_A, *g_B, *g_C, *g_R;
#else
#ifdef _USE_SSE
__declspec(align(16)) double *g_A, *g_B, *g_C, *g_R;
#else
double *g_A, *g_B, *g_C, *g_R;
#endif
#endif
#endif
int KERNEL = 0;
...
// 三重ループ
void cpu_kernel_mnk(int m, int n, int k, real *_a, real *_b, real *_c)
{
  int x, y, z;
...
// ループ交換
void cpu_kernel_nmk(int m, int n, int k, real *_a, real *_b, real *_c)
{
  int x, y, z;
#pragma omp parallel for private(x,y,z)
  for(y=0; y<n; y++){
...
// ループ交換
void cpu_kernel_nkm(int m, int n, int k, real *_a, real *_b, real *_c)
{
  int x, y, z;
#pragma omp parallel for private(x,y,z)
  for(y=0; y<n; y++){
...
void cpu_kernel(int kernel, int _m, int _n, int _k, real *_a, real *_b, real *_c)
{
  switch(kernel){
  default: exit(-1); break;
  case 0:
	cpu_kernel_mnk(_m, _n, _k, _a, _b, _c);
	break;
  case 1:
	cpu_kernel_nmk(_m, _n, _k, _a, _b, _c);
	break;
  case 2:
	cpu_kernel_nkm(_m, _n, _k, _a, _b, _c);
	break;
#ifndef _NO_MKL
  case 50:
    cblas_dgemm
...
int main(int argc, char** argv)
{
  ...
  g_A = (double*)_malloc_2M(sizeof(double)*size_m*size_k);
  g_B = (double*)_malloc_2M(sizeof(double)*size_k*size_n);
  g_C = (double*)_malloc_2M(sizeof(double)*size_m*size_n);
  g_R = (double*)_malloc_2M(sizeof(double)*size_m*size_n);
#else
  g_A = (real*)malloc(sizeof(real)*size_m*size_k);
  g_B = (real*)malloc(sizeof(real)*size_k*size_n);
  g_C = (real*)malloc(sizeof(real)*size_m*size_n);
  g_R = (real*)malloc(sizeof(real)*size_m*size_n);
#ifndef _NO_MKL
#endif
#endif
  //MyPrintf("A %x\nB %x\nC %x\n", g_A, g_B, g_C);
  ...
  for(y=0; y<size_m; y++){
	for(x=0; x<size_k; x++){
	  g_A[y*size_k+x] = frand();
	}
  }
  for(y=0; y<size_k; y++){
	for(x=0; x<size_n; x++){
	  g_B[y*size_n+x] = frand();
	}
  }
  for(y=0; y<size_m; y++){
	for(x=0; x<size_n; x++){
	  g_C[y*size_n+x] = frand();
	}
  }
  for(y=0; y<size_m; y++){
	for(x=0; x<size_n; x++){
	  g_R[y*size_n+x] = g_C[y*size_n+x];
	}
  }
  //MyPrintf("done\n");
  ...
  double dBegin, dEnd;
  cpu_kernel(KERNEL,size_m,size_n,size_k,g_A, g_B, g_C);
  for(i=0;i<LOOPS;i++){
  ...
  MyPrintf("performance: kernel %d, size %d %d %d, total %f sec, average %f sec, min %f sec, max %f sec, %f Mflops",
		   KERNEL, size_m, size_n, size_k, dSecSum, dSecSum/(double)LOOPS, dSecMin, dSecMax,
		   (double)(LOOPS*2.0*(double)size_m*(double)size_n*(double)size_k)/dSecSum/1000.0/1000.0);
...
#pragma omp parallel for reduction(+:tmp)
  ...
  /*
  {
	FILE *F;
	char filename[0xff];
	snprintf(filename, 0xff, "result_%d.dat", KERNEL);
	F = fopen(filename,"w");
	for(i=0; i<size_m*size_n; i++){
	  fprintf(F, "%e\n", g_C[i]);
	}
  ...
  */
#ifdef _USE_AVX3
  munmap(g_A, (sizeof(double)*(size_m*size_k)+4096));
  munmap(g_B, (sizeof(double)*(size_k*size_n)+4096));
  munmap(g_C, (sizeof(double)*(size_m*size_n)+4096));
  munmap(g_R, (sizeof(double)*(size_m*size_n)+4096));
#else
  free(g_A);
  free(g_B);
  free(g_C);
